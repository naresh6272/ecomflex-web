...
function buildLightSculpture() {
  ...
  // Premium "crystal" glass. Previous version used a near-white tint at
  // high transparency sitting on a near-white page background — on a
  // light background, near-white transparent glass has almost no visible
  // contrast and effectively disappears. A visible warm-champagne tint
  // plus a brighter, more opaque edge frame gives the panels real
  // definition regardless of what's behind them.
  const glassMat = () => new THREE.MeshPhysicalMaterial({
    color: 0xF3E6C8, roughness: 0.05,
    metalness: 0, clearcoat: 1, clearcoatRoughness: 0.04,
    transparent: true, opacity: 0.42, side: THREE.DoubleSide,
  })
  // No longer uses real `transmission` — with 8 panels now on screen,
  // even 2 transmissive ones re-triggered a real, repeatable "hanging"
  // complaint. Transmission forces an extra full-scene render pass every
  // frame; removing it entirely is the only way to guarantee it can't be
  // the cause. This still reads as glass via clearcoat + low roughness.
...
self.onmessage = (e: MessageEvent) => {
  ...
  if (type === 'init') {
    ...
    if (logoTex && logoCanvasColor) {
      logoTex.image = logoCanvasColor as unknown as HTMLCanvasElement
    ...
    // Color version — draw ImageBitmap onto OffscreenCanvas as-is (dark theme, unchanged)
    const ocColor = new OffscreenCanvas(bitmap.width, bitmap.height)
    const ctxColor = ocColor.getContext('2d')
    if (!ctxColor) return  // browser doesn't support 2d on OffscreenCanvas
    ctxColor.drawImage(bitmap, 0, 0)
    logoCanvasColor = ocColor

    const tex = new THREE.CanvasTexture(ocColor as unknown as HTMLCanvasElement)
